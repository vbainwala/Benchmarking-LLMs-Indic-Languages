...
for i in range(100):
    ...
    # print(decoded[i])
    results = meteor.compute(predictions=[decoded[i]], references=[target[i]])
...
with open('/Users/vareeshbainwala/Documents/Coursework/Codes/COMS6998/Machine_Translation/Experiment_Results/indicbart_hien.csv','w') as file:
...
'''Translating the text from Hindi to English using the translation mBart large gave from english to hindi'''
source_bw = []
target_bw = []
...
for i in range(len(source_bw)):
    ...
    # print(decoded[i])
    results = meteor.compute(predictions=[decoded_bw[i]], references=[target_bw[i]])
...
with open('/Users/vareeshbainwala/Documents/Coursework/Codes/COMS6998/Machine_Translation/Experiment_Results/indicbart_hien_backtranslation.csv','w') as file:
    fieldnames = ['Source','Target','Prediction','Meteor','Bleu']
    csvwriter = csv.DictWriter(file,fieldnames=fieldnames)
    csvwriter.writeheader()
    for i in range(len(source_bw)):
